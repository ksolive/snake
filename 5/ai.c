...
int eval2(game_t* game, int sx, int sy, snake_t* snake2) {
	int dis1[N][N], dis2[N][N];
	bfs(sx, sy, game, dis1);
	bfs(snake2->head->row, snake2->head->col, game, dis2);
	int cnt1 = 0, cnt2 = 0;
...
direction_t Difficult(game_t *game, snake_t *snake1, snake_t *snake2) {
	int x = snake1->head->row, y = snake1->head->col;
	direction_t cand[10];
	...
	for (int i = 0; i < 4; i++) 
	{
		...
		if (*board_cell(game->board, tx, ty) == CELL_OPEN)
		{
			int score = eval2(game, tx, ty, snake2);
			if (score > mx_score)
			{
				mx_score = score;
				cand[0] = i;
...
direction_t get_action1(game_t *game) {
	return Difficult(game, game->snake1, game->snake2);
